...
export default function Achievements() {
  // Your professional milestones, competitive tracking, and certificates
  const achievementsData = [
    {
      ...
      link: "#" // Add your certificate URL if available
    },
    {
    ...
    <section className="relative min-h-screen py-6 animate-fadeIn">
      ...
      <div className="mb-12 text-center lg:text-left">
        ...
        <h2 className="text-3xl sm:text-4xl font-black italic uppercase tracking-tight mt-1 text-white">
          Key <span className="text-[#58a6ff] not-italic tracking-normal normal-case drop-shadow-[0_0_15px_rgba(88,166,255,0.15)]">Achievements</span>
        </h2>
        <div className="h-[3px] w-20 bg-gradient-to-r from-[#58a6ff] to-transparent mt-3 rounded-full mx-auto lg:mx-0" />
      </div>

      {/* TWO-COLUMN GRID ASSEMBLY FOR DETAILED LAYOUTS */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-5xl">
        {achievementsData.map((item, index) => {
          ...
          return (
            <div 
              key={index}
              className="group relative bg-[#161b22] border border-[#30363d] rounded-2xl p-6 sm:p-8 flex flex-col justify-between overflow-hidden shadow-xl transition-all duration-300 hover:border-[#58a6ff]/40 hover:shadow-2xl"
            >
              {/* Subtle visual top glow effect displayed uniquely on hover */}
              ...
              <div>
                {/* CARD UPPER BAR CONTENT */}
                <div className="flex items-start justify-between gap-4 mb-6">
                  
                  {/* Dynamic Achievement Icon Shield */}
                  <div className="flex items-center justify-center w-12 h-12 rounded-xl bg-[#0d1117] border border-[#30363d] text-gray-500 group-hover:text-[#58a6ff] group-hover:border-[#58a6ff]/20 transition-all duration-300 shadow-inner">
                    <IconComponent className="w-6 h-6" />
                  </div>

                  {/* Category Status Flag Badge */}
                  <span className={`px-2.5 py-1 rounded-md text-[10px] uppercase font-mono font-bold tracking-wider border ${item.tagColor}`}>
                    {item.category}
                  </span>
                </div>

                {/* TEXT AREA DESCRIPTION METRICS */}
                <div className="space-y-2">
                  <span className="text-[11px] font-mono font-semibold tracking-widest text-gray-500 uppercase block">
                    // {item.metric}
                  </span>
                  
                  <h3 className="text-xl font-bold tracking-tight text-white group-hover:text-[#58a6ff] transition-colors duration-200 flex items-center gap-2">
              ...
              {/* CARD LOWER INTERACTIVE BLOCK ANCHOR */}
